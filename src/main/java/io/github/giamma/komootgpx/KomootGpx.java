...
public class KomootGpx {
    ...
    private static Double parseDoubleOption(CommandLine cmd, String option) throws ParseException {
        String value = cmd.getOptionValue(option);
        if (value == null) return null;
        try {
            return Double.parseDouble(value);
        } catch (NumberFormatException e) {
            throw new ParseException("Invalid number format for --" + option + ": " + value);
        }
    }
    
    private static Integer parseIntOption(CommandLine cmd, String option) throws ParseException {
        String value = cmd.getOptionValue(option);
        if (value == null) return null;
        try {
            return Integer.parseInt(value);
        } catch (NumberFormatException e) {
            throw new ParseException("Invalid integer format for --" + option + ": " + value);
        }
    }
    
    private static Double parseDoubleOptionWithDefault(CommandLine cmd, String option, double defaultValue) throws ParseException {
        if (!cmd.hasOption(option)) {
            return null; // Option not specified
        }
        
        String value = cmd.getOptionValue(option);
        if (value == null || value.isEmpty()) {
            return defaultValue; // Option specified without value, use default
        }
    ...
    private static Integer parseIntOptionWithDefault(CommandLine cmd, String option, int defaultValue) throws ParseException {
        if (!cmd.hasOption(option)) {
            return null; // Option not specified
        }
